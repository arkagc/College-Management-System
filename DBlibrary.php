<?php
    ...
    /*public function registerUser($name, $email, $phone,$pwd,$otp) 
	{
			
		//check if user already exists
      $stmt = $this->conn->prepare("select phone from users where phone=?");
	   ...
	   if($num_rows==1){
		   return 2;
            ...
            // insert query for mysqli 
			
        $stmt=$this->conn->prepare("INSERT INTO users(name, email,phone,password) values(?,?,?,?)");
			//echo "name=".$name." email=".$email." phone= ".$phone." password=" .$pwd;
            ...
            if ($result) {
                return 0; // User successfully inserted
            } else {
                // Failed to create user
                return 1;
            } 
	}
		 public function loginUser($mobile,$pwd) 
	 {
        $stmt = $this->conn->prepare("select phone,password from users where phone=? and password=?");
...
public function myPost($user_id){
    ...
    public function groupMember(){
        ...
        //while($mem=mysqli_fetch_array($member)){
          //  echo $mem['first_name'];
            //echo $mem['middle_name'];
            //echo $mem['last_name'];
        //}
